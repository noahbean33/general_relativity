## Rn.py
## SageManifolds implementation of Euclidean R^n as classes (Python equivalent of Rn.sage)
...
class R3(object):
    def __init__(self):
        ...
        self.transit_sph_to_cart.set_inverse(Sphnorm,
            atan2(sqrt(sum([self.cart_ch_U[i]**2 for i in range(1, 3)])), self.cart_ch_U[3]),
            atan2(self.cart_ch_U[2], self.cart_ch_U[1]))
        ...
        self.transit_cyl_to_cart.set_inverse(
            sqrt(self.cart_ch_U[1]**2 + self.cart_ch_U[2]**2),
            atan2(self.cart_ch_U[2], self.cart_ch_U[1]),
            self.cart_ch_U[3])
...
class Rn(object):
    def __init__(self, n):
        ...
        if n == 2:
            ...
            self.sph_ch = self.U.chart(
                r'rh:(0,+oo):\rho ' +
                r" ".join([r"th"+str(i)+r":(0,pi)" for i in range(1, n+1-2)]) +
                r' ph:(0,2*pi):\phi')
            sphs = [self.sph_ch[i[0]] for i in self.M.index_generator(1)]
            self.transit_sph_to_cart = self.sph_ch.transition_map(self.cart_ch_U,
                [sphs[0]*prod([sin(sphs[i]) for i in range(1, n-1)])*cos(sphs[-1]),
                 sphs[0]*prod([sin(sphs[i]) for i in range(1, n-1)])*sin(sphs[-1])] +
                [sphs[0]*prod([sin(sphs[i]) for i in range(1, j)])*cos(sphs[j])
                 for j in range(n-2, 1, -1)] +
                [sphs[0]*cos(sphs[1])])
            gen_transit_list_sph = (
                [sqrt(sum([xis[i]**2 for i in range(len(xis))]))] +
                [atan2(sqrt(sum([xis[i]**2 for i in range(j)])), xis[j])
                 for j in range(n-1, 1, -1)] +
                [atan2(xis[1], xis[0])])
            self.transit_sph_to_cart.set_inverse(*gen_transit_list_sph)

            # cylindrical coordinates
            self.cyl_ch = self.U.chart(
                r'r:(0,+oo) ' +
                r" ".join([r"the"+str(i)+r":(0,pi)" for i in range(1, n+1-3)]) +
                r' phi:(0,2*pi):\varphi z')
            cyls = [self.cyl_ch[i[0]] for i in self.M.index_generator(1)]
            self.transit_cyl_to_cart = self.cyl_ch.transition_map(self.cart_ch_U,
                [cyls[0]*prod([sin(cyls[i]) for i in range(1, n-2)])*cos(cyls[-2]),
                 cyls[0]*prod([sin(cyls[i]) for i in range(1, n-2)])*sin(cyls[-2])] +
                [cyls[0]*prod([sin(cyls[i]) for i in range(1, j)])*cos(cyls[j])
                 for j in range(n-3, 1, -1)] +
                [cyls[0]*cos(cyls[1]), cyls[-1]])
            gen_transit_list_cyl = (
                [sqrt(sum([xis[i]**2 for i in range(len(xis)-1)]))] +
                [atan2(sqrt(sum([xis[i]**2 for i in range(j)])), xis[j])
                 for j in range(n-2, 1, -1)] +
                [atan2(xis[1], xis[0]), xis[n-1]])
...
def make_material_der(u, ch):
    ...
    X = sum([uedcomp[i[0]-1]*ch.frame()[i[0]]
             for i in ch.domain().manifold().index_generator(1)])
    return X
